...
final class Pasthis {
    ...
    private function render () {
        ...
        print '</head>';
        print '<div class="container" style="margin-top: 3%">';
        print '<h1>Pasthis</h1>';
        print '<div id="common">';
        print '<h4>Partage de presse-papier</h4>';
        print '<body>';
        while (list (, $ct) = each ($this->contents))
            print $ct;
        print '<div id="footer">';
        print 'Propulsé par <a href="http://pasthis.moulecorp.org">pasthis</a> - ';
        print '<a href="./pasthis.pl">outil en ligne de commande</a>';
        print '</div>';
        print '</div>';
        print '</body>';
        print '</html>';
    ...
    private function remaining_time ($timestamp) {
        ...
        $ret = 'Expire dans '.$format ($interval->days, 'jours');
        if ($interval->days < 31) {
            $ret .= $format ($interval->h, 'heures');
            if ($interval->d === 0) {
                $ret .= $format ($interval->i, 'minutes');
                if ($interval->h === 0)
                    $ret .= $format ($interval->s, 'secondes');
            }
        }
        return rtrim ($ret).'.';
    }
    ...
    function prompt_paste () {
        $this->add_content (
            '<form method="post" action=".">
                ...
                <select name="d" id="d">
                    <option value="-2">détruire après lecture</option>
                    <option value="600">10 minutes</option>
                    <option value="3600">1 heure</option>
                    <option value="86400" selected="selected">1 jour</option>
                    <option value="604800">1 semaine</option>
                    <option value="-1">éternelle</option>
                </select>
                <input type="text" id="ricard" name="ricard"
                        placeholder="Do not fill me!" />
                <input type="submit" value="Envoyer">
                <span id="left">
                <input type="checkbox" id="wrap" name="wrap"> Casser les lignes trop longues<br />
                <input type="checkbox" id="highlighting" name="highlighting"> Activer la coloration syntaxique<br />
                </span>
    ...
    function show_paste ($param) {
        ...
        if ($fail) {
            $this->add_content ('<p>Pas de presse-papier pour cet ID :(</p>');
            $this->prompt_paste ();
        } else {
            ...
            if ($is_raw) {
                ...
                $this->add_content (
                    '<div id="links">
                         <a href="./'.$id.'@raw">Données brutes</a> - <a href="./">Nouveau</a>
